...
function getPreviewWidth(width: number, height: number) {
  const ratio = width / height;
  const previewWidth = Math.round(Math.min(360, Math.max(185, ratio * 290)));
...
export function PhotoGallery({ photos }: PhotoGalleryProps) {
  ...
  const previous = () => setActiveIndex((current) => (current === null ? null : current === 0 ? photos.length - 1 : current - 1));
  const next = () =>
    setActiveIndex((current) => (current === null ? null : current === photos.length - 1 ? 0 : current + 1));
  ...
  return (
    <>
      <div className="min-w-0 max-w-full space-y-5">
        <div className="grid gap-4 border-y border-emerald-900/10 py-5 sm:py-6 xl:grid-cols-[1fr_auto] xl:items-end">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 text-[#bf9130]">
              ...
              <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-700 sm:text-sm sm:tracking-[0.22em]">
                {landingContent.gallerySection.eyebrow}
              </p>
            </div>
            <p className="mt-3 text-2xl font-semibold leading-tight text-[#1e1b18] sm:text-4xl">{landingContent.gallerySection.title}</p>
            <p className="mt-3 text-sm leading-7 text-[#1e1b18]/70 sm:text-base sm:leading-8">{landingContent.gallerySection.description}</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3 xl:min-w-[300px]">
            <div className="rounded-[18px] bg-white/70 px-3 py-3 text-center shadow-[0_14px_30px_rgba(18,47,37,0.06)] backdrop-blur">
              <p className="text-xl font-semibold text-emerald-900 sm:text-3xl">{photos.length}</p>
              <p className="mt-1 text-[10px] leading-4 text-[#1e1b18]/65 sm:text-xs sm:leading-5">фотографий</p>
            </div>
            <div className="rounded-[18px] bg-white/70 px-3 py-3 text-center shadow-[0_14px_30px_rgba(18,47,37,0.06)] backdrop-blur">
              <p className="text-xl font-semibold text-emerald-900 sm:text-3xl">Живые</p>
              <p className="mt-1 text-[10px] leading-4 text-[#1e1b18]/65 sm:text-xs sm:leading-5">без постановки</p>
            </div>
            <div className="rounded-[18px] bg-white/70 px-3 py-3 text-center shadow-[0_14px_30px_rgba(18,47,37,0.06)] backdrop-blur">
        ...
        <div className="marquee-viewport marquee-viewport-compact-fade w-full max-w-full pb-4">
          <div
          ...
          >
            {duplicatedPhotos.map((photo, index) => (
              <button
                ...
                style={{ width: getPreviewWidth(photo.width, photo.height) }}
                className={cn(
                ...
                aria-label={`Открыть ${photo.label}`}
              >

...
import { loadAdminClients } from '../../lib/backendApi';
import { STUDIO_SUBSCRIPTIONS } from '../../constants/subscriptions';
...
export function AdminStudents({ groups }: AdminStudentsProps) {
  ...
  const [selectedStudent, setSelectedStudent] = useState<AdminStudentViewModel | null>(null);
  const [noteText, setNoteText] = useState('');
  const addDialogSubscriptions = STUDIO_SUBSCRIPTIONS.map((item) => ({
    id: item.id,
    name: item.name,
    classes: item.classesCount ?? 0,
    price: item.price,
  }));
  const editDialogSubscriptions = STUDIO_SUBSCRIPTIONS.map((item) => ({
    id: item.id,
    name: item.name,
    lessonsCount: item.classesCount ?? 0,
  }));
    ...
    const hydrateFromBackend = async () => {
      try {
        ...
        const mappedStudents: AdminStudentViewModel[] = serverClients.map((client) => {
          ...
          const amount = Number(client.payment?.amount ?? client.subscriptionAmount ?? 0);
          const rawPaymentStatus = client.payment?.status ?? client.paymentStatus;
          const paymentStatus =
          ...
          return {
            id: client.childId,
            name: client.childFullName || 'Без имени',
            birthDate,
            age: calculateAge(birthDate),
            groupId: '',
            groupName: 'Не назначена',
            parentName: client.parentName || 'Без имени',
            parentEmail: '',
            ...
            remainingClasses: 0,
            lastPaymentDate: client.payment?.paidAt ? new Date(client.payment.paidAt) : createdAt,
  ...
  const handleOpenNoteDialog = (student: AdminStudentViewModel) => {
  ...
  const handleSaveNote = () => {
    if (selectedStudent) {
      setStudents((prev) =>
        prev.map((student) =>
          student.id === selectedStudent.id ? { ...student, notes: noteText } : student,
        ),
      );
      setSelectedStudent((prev) => (prev ? { ...prev, notes: noteText } : prev));
    }
  ...
  const handleStudentCreated = (newStudent: AdminStudentViewModel) => {
    setStudents((prev) => [newStudent, ...prev]);
    setParentsData((prev) => {
      const exists = prev.some((parent) => parent.phone === newStudent.parentPhone);
      if (exists) {
        return prev;
      }
      return [
        {
          id: `parent-${newStudent.id}`,
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-scale-in">
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-3xl text-[#133C2A]">{students.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-3xl text-[#133C2A]">
                  {students.filter((s) => s.paymentStatus === 'overdue' || s.paymentStatus === 'pending').length}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <EditStudentDialog
        ...
        groups={groups}
        parents={parentsData}
        subscriptions={editDialogSubscriptions}
      />
      ...
      <AddStudentDialog
        ...
        groups={groups}
        parents={parentsData}
        subscriptions={addDialogSubscriptions}

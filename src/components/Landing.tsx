...
const ageOptions = ['3–5 лет', '6–8 лет', '9–15 лет', 'PRO'];
const interestOptions = ['Пробное занятие', 'Хобби-группа', 'PRO-состав', 'Нужна консультация'];
...
function validateLeadForm(form: LeadFormState): LeadErrors {
  ...
  if (!form.childAgeRange.trim()) {
    errors.childAgeRange = 'Выберите возраст ребёнка';
  }
  if (!form.interestFormat.trim()) {
    errors.interestFormat = 'Выберите формат интереса';
  }
  if (!form.phone.trim()) {
...
export function Landing({ onLogin, onGuestBrowse, onAddTask, onAddNotification }: LandingProps) {
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const childDisplayName = form.childFullName.trim() || `Ребёнок ${form.childAgeRange}`;

    const task: Task = {
      id: `landing-task-${now}`,
      title: `Новая заявка: ${form.parentFullName}`,
      description: [
        `Ребёнок: ${childDisplayName}`,
        `Возраст: ${form.childAgeRange}`,
        `Интерес: ${form.interestFormat}`,
      ...
      relatedChildName: childDisplayName,
    ...
    const notification: Notification = {
      ...
      message: `${form.parentFullName} оставил(а) заявку: ${form.interestFormat}, ${form.childAgeRange}.`,
    ...
    try {
      await createLandingLead({
        ...
        child_full_name: childDisplayName,
        ...
        preferred_schedule: `${form.childAgeRange}; ${form.interestFormat}`,
  ...
  return (
    <div className="landing-v2 rich-ui min-h-screen">
      ...
      <main>
          ...
          <div className="mx-auto grid w-full max-w-7xl items-start gap-6 px-4 pb-14 pt-2 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:gap-10 lg:pb-20 lg:pt-8">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="hidden max-w-2xl space-y-2 text-base leading-7 text-[#1e1b18]/75 sm:text-lg sm:leading-8 lg:block">
              ...
              <div className="grid grid-cols-2 gap-2 rounded-[26px] border border-white/70 bg-white/75 p-3 shadow-sm backdrop-blur sm:grid-cols-4">
                {ageOptions.map((option) => (
                  <button
                    key={option}
                    type="button"
                    onClick={() => {
                      handleFieldChange('childAgeRange', option);
                      scrollToSection('#form');
                    }}
                    className="rounded-2xl border border-emerald-900/10 bg-[#f8f2e8] px-3 py-3 text-sm font-medium text-[#1e1b18] transition hover:border-emerald-700/35 hover:bg-white"
                  >
                    {option}
                  </button>
                ))}
        ...
        <section id="form" className="scroll-mt-24 py-12 sm:py-14">
          <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[0.85fr_1.15fr]">
            ...
            <form onSubmit={handleFormSubmit} className="rounded-[28px] border border-white/70 bg-white/85 p-5 shadow-sm backdrop-blur sm:p-8" noValidate>
              <div className="rounded-[24px] border border-emerald-900/10 bg-[#f8f2e8] p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">1. Возраст ребёнка</p>
                <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {ageOptions.map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => handleFieldChange('childAgeRange', option)}
                      className={`rounded-2xl border px-3 py-3 text-sm transition ${
                        form.childAgeRange === option
                          ? 'border-emerald-800 bg-emerald-900 text-white'
                          : 'border-emerald-900/10 bg-white text-[#1e1b18]'
                      }`}
                    >
                      {option}
                    </button>
                ...
                {formErrors.childAgeRange ? <p className="mt-2 text-sm text-red-700">{formErrors.childAgeRange}</p> : null}
              </div>

              <div className="mt-4 rounded-[24px] border border-emerald-900/10 bg-white/70 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">2. Формат</p>
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  {interestOptions.map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => handleFieldChange('interestFormat', option)}
                      className={`rounded-2xl border px-4 py-3 text-left text-sm transition ${
                        form.interestFormat === option
                          ? 'border-emerald-800 bg-emerald-900 text-white'
                          : 'border-emerald-900/10 bg-white text-[#1e1b18]'
                      }`}
                    >
                ...
                {formErrors.interestFormat ? <p className="mt-2 text-sm text-red-700">{formErrors.interestFormat}</p> : null}
              </div>

              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <label className="block">
                  <span className="mb-2 block text-sm font-medium text-[#1e1b18]">Имя родителя</span>
                  <input
                ...
                <label className="block">
                  <span className="mb-2 block text-sm font-medium text-[#1e1b18]">Телефон</span>
                  <input
                    value={form.phone}
              ...
              <details className="mt-4 rounded-[24px] border border-emerald-900/10 bg-white/60 p-4">
                <summary className="cursor-pointer text-sm font-medium text-[#1e1b18]">Дополнить анкету сейчас</summary>
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <label className="block">
                  ...
                  <label className="block">
                    <span className="mb-2 block text-sm font-medium text-[#1e1b18]">Дата рождения</span>
                    <input
                ...
                <label className="mt-4 block">
                  <span className="mb-2 block text-sm font-medium text-[#1e1b18]">Медицинские ограничения</span>
                  <textarea
                    value={form.medicalRestrictions}
                    rows={3}
                    placeholder="Если ограничений нет, можно оставить пустым"
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <label className="block">
                    <span className="mb-2 block text-sm font-medium text-[#1e1b18]">Опыт занятий</span>
                    <input
                      value={form.previousActivities}
                      placeholder="Например: гимнастика, танцы, нет опыта"
                      onChange={(event) => handleFieldChange('previousActivities', event.target.value)}
                      className="w-full rounded-2xl border border-emerald-900/10 bg-white px-4 py-3.5 text-sm text-[#1e1b18] outline-none transition focus:border-emerald-500"
                    />
                  </label>
                  <label className="block">
                    <span className="mb-2 block text-sm font-medium text-[#1e1b18]">Откуда узнали</span>
                    <input
                      value={form.discoverySource}
                      placeholder="Рекомендация, Telegram, Instagram"
      ...
      >
        <CtaButton label="Записаться на пробное" targetId="form" className="flex w-full justify-center py-4 text-base shadow-lg" />
      </div>
      </div>
    </div>
  );
